...
import { multiplesDivisorsQuestions } from './data/multiplesDivisorsQuestions';
import { speedQuestions } from './data/speedQuestions';
import { workNewtonQuestions } from './data/workNewtonQuestions';
...
import { SpeedAnswerExplanation } from './components/SpeedAnswerExplanation';
import { WorkNewtonAnswerExplanation } from './components/WorkNewtonAnswerExplanation';
import { ProfitLossAnswerExplanation } from './components/ProfitLossAnswerExplanation';
import { JudgementReasoningAnswerExplanation } from './components/JudgementReasoningAnswerExplanation';
import { judgementReasoningQuestions } from './data/judgementReasoningQuestions';
import type { Question } from './types';

const questions = [...(questionsJson as Question[]), ...saltwaterQuestions, ...sequenceQuestions, ...multiplesDivisorsQuestions, ...speedQuestions, ...workNewtonQuestions, ...profitLossQuestions, ...judgementReasoningQuestions];
type CategoryId = 'factorization'|'saltwater-alligation'|'sequence-patterns'|'multiples-divisors'|'speed-patterns'|'work-newton-patterns'|'profit-loss-patterns'|'judgement-reasoning-patterns';
type Screen = 'home'|'categories'|'modes'|'session'|'quiz';
const titleFor:Record<CategoryId,string> = { factorization:'因数分解・公式暗記', 'saltwater-alligation':'食塩水・濃度｜天秤算', 'sequence-patterns':'数列｜規則の見つけ方', 'multiples-divisors':'倍数と約数｜型の見分け方', 'speed-patterns':'速さ｜型の見分け方', 'work-newton-patterns':'仕事算・ニュートン算｜最小公倍数', 'profit-loss-patterns':'損益算｜原価100裏技', 'judgement-reasoning-patterns':'判断推理｜型の見分け方' };
...
const maxQuestionsFor = (categoryId:CategoryId, mode:LearningModeId, fallback?:number) => mode === 'random' && categoryId === 'speed-patterns' ? 15 : fallback;
...
export function AppNew() {
  ...
  const startMode = (nextMode:LearningModeId) => {
    ...
    const ids = validSaved ? saved.questionIds : (definition.shuffle ? shuffled(source).slice(0, maxQuestionsFor(categoryId, nextMode, definition.maxQuestions) ?? source.length) : source).map((q) => q.id);
  ...
  const restartMode = () => {
    ...
    const ids = (definition.shuffle ? shuffled(source).slice(0, maxQuestionsFor(categoryId, modeId, definition.maxQuestions) ?? source.length) : source).map((item) => item.id);
  ...
  const modeCounts = Object.fromEntries(learningModes.map((mode) => [mode.id, Math.min(candidates(categoryId, mode.id).length, maxQuestionsFor(categoryId, mode.id, mode.maxQuestions) ?? Number.POSITIVE_INFINITY)]));
  ...
  if (screen === 'categories') return <main className="app"><Header title="カテゴリを選ぶ" onBack={() => go('home')} />{(['factorization','saltwater-alligation','sequence-patterns','multiples-divisors','speed-patterns','work-newton-patterns','profit-loss-patterns','judgement-reasoning-patterns'] as CategoryId[]).map((id) => <button key={id} className="selection-card" onClick={() => selectCategory(id)}><span className="card-icon">{id==='factorization'?'🟣':id==='saltwater-alligation'?'⚖️':id==='sequence-patterns'?'🔢':id==='multiples-divisors'?'🔗':id==='speed-patterns'?'🚲':id==='work-newton-patterns'?'🛠️':id==='profit-loss-patterns'?'💰':'🧩'}</span><span className="card-copy"><b>{titleFor[id]}</b><small>{id==='saltwater-alligation'?'天秤の型を見つける':id==='sequence-patterns'?'差・比・交互を順番に見る':id==='multiples-divisors'?'余り・不足・分けるを見分ける':id==='speed-patterns'?'向かう・追いつく・往復を見分ける':id==='work-newton-patterns'?'最小公倍数で分数なしに解く':id==='profit-loss-patterns'?'原価100で％を整数にする':id==='judgement-reasoning-patterns'?'年齢・集合・時計の型を見分ける':'公式の型を見分ける'}</small><em>全 {categoryQuestions(id).length}問</em></span><span className="card-arrow">›</span></button>)}</main>;
  ...
  if (answered && question.speed) return <SpeedAnswerExplanation question={question} isCorrect={correct} index={index} total={activeQuestions.length} menu={menu} onNext={next} />;
  if (answered && question.workNewton) return <WorkNewtonAnswerExplanation question={question} isCorrect={correct} index={index} total={activeQuestions.length} menu={menu} onNext={next} />;
  if (answered && question.profitLoss) return <ProfitLossAnswerExplanation question={question} isCorrect={correct} index={index} total={activeQuestions.length} menu={menu} onNext={next} />;
  if (answered && question.judgementReasoning) return <JudgementReasoningAnswerExplanation question={question} isCorrect={correct} index={index} total={activeQuestions.length} menu={menu} onNext={next} />;
  const typeName = question.saltwater?.problemPattern ?? question.sequence?.problemPattern ?? question.multiplesDivisors?.problemPattern ?? question.speed?.problemPattern ?? question.workNewton?.problemPattern ?? question.profitLoss?.problemPattern ?? question.judgementReasoning?.problemPattern ?? '公式の型';
