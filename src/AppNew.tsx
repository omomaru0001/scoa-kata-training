...
const questions = [...(questionsJson as Question[]), ...saltwaterQuestions, ...sequenceQuestions, ...multiplesDivisorsQuestions, ...speedQuestions, ...workNewtonQuestions];
type CategoryId = 'factorization'|'saltwater-alligation'|'sequence-patterns'|'multiples-divisors'|'speed-patterns'|'work-newton-patterns';
type Screen = 'home'|'categories'|'modes'|'session'|'quiz';
const titleFor:Record<CategoryId,string> = { factorization:'因数分解・公式暗記', 'saltwater-alligation':'食塩水・濃度｜天秤算', 'sequence-patterns':'数列｜規則の見つけ方', 'multiples-divisors':'倍数と約数｜型の見分け方', 'speed-patterns':'速さ｜型の見分け方', 'work-newton-patterns':'仕事算・ニュートン算｜LCM裏技' };
...
export function AppNew() {
  ...
  if (screen === 'categories') return <main className="app"><Header title="カテゴリを選ぶ" onBack={() => go('home')} />{(['factorization','saltwater-alligation','sequence-patterns','multiples-divisors','speed-patterns','work-newton-patterns'] as CategoryId[]).map((id) => <button key={id} className="selection-card" onClick={() => selectCategory(id)}><span className="card-icon">{id==='factorization'?'🟣':id==='saltwater-alligation'?'⚖️':id==='sequence-patterns'?'🔢':id==='multiples-divisors'?'🔗':id==='speed-patterns'?'🚲':'🛠️'}</span><span className="card-copy"><b>{titleFor[id]}</b><small>{id==='saltwater-alligation'?'天秤の型を見つける':id==='sequence-patterns'?'差・比・交互を順番に見る':id==='multiples-divisors'?'余り・不足・分けるを見分ける':id==='speed-patterns'?'向かう・追いつく・往復を見分ける':id==='work-newton-patterns'?'LCMで分数なしに解く':'公式の型を見分ける'}</small><em>全 {categoryQuestions(id).length}問</em></span><span className="card-arrow">›</span></button>)}</main>;
  ...
  const typeName = question.saltwater?.problemPattern ?? question.sequence?.problemPattern ?? question.multiplesDivisors?.problemPattern ?? question.speed?.problemPattern ?? question.workNewton?.problemPattern ?? '公式の型';
