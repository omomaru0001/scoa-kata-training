...
export function WorkNewtonAnswerExplanation({ question, isCorrect, index, total, menu, onNext }:{question:Question;isCorrect:boolean;index:number;total:number;menu:QuizBackActions;onNext:()=>void}) {
  ...
  return <main className="app quiz-screen"><header className="screen-header"><BackMenu {...menu}/><h1>仕事算・ニュートン算</h1><span>{index+1} / {total}</span></header>
    <p className="session-label">答えへ進む順番</p><section className="answer-feedback speed-answer">
      ...
      <p className="mistake">⚠ 間違えやすいポイント：{question.mistakeReason}</p><details><summary>なぜそうなる？</summary><p>{question.deepExplanation}</p></details>
