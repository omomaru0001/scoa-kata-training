...
import { MultiplesDivisorsDiagram } from './MultiplesDivisorsDiagram';
import { SpeedDiagram } from './SpeedDiagram';
import { WorkNewtonDiagram } from './WorkNewtonDiagram';
import { ProfitLossDiagram } from './ProfitLossDiagram';
export function Diagram({question,revealAnswer=false}:{question:Question;revealAnswer?:boolean}) { const d=question.diagramData;
 if(question.diagramType==='saltwater-alligation' && question.saltwater) return <SaltwaterAlligationDiagram data={question.saltwater} revealAnswer={revealAnswer} />;
 if(question.diagramType==='sequence-pattern' && question.sequence) return <SequencePatternDiagram data={question.sequence} />;
 if(question.diagramType==='multiples-divisors' && question.multiplesDivisors) return <MultiplesDivisorsDiagram data={question.multiplesDivisors} />;
 if(question.diagramType==='speed' && question.speed) return <SpeedDiagram data={question.speed} />;
 if(question.diagramType==='work-newton' && question.workNewton) return <WorkNewtonDiagram data={question.workNewton} />;
 if(question.diagramType==='profit-loss' && question.profitLoss) return <ProfitLossDiagram data={question.profitLoss} />;
