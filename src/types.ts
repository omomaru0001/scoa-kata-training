...
export type JudgementReasoningType = 'age-multiple'|'age-sum'|'age-three'|'divisor-count'|'square-tiling'|'square-difference'|'set-simple'|'set-ratio'|'cross-table'|'ratio-change'|'tree-planting'|'clock'|'judgement-identify';
...
export type JudgementReasoningData = { problemPattern:string; readingClues:string[]; clueReason:string; characterTip:string; modeIds:('memorize'|'blank'|'identify'|'substitute'|'practice')[]; knownFacts:string[]; validationData:{kind:JudgementReasoningType;answerText:string}; };
export type Question = { id:string; categoryId:string; subcategoryId:string; typeId:FormulaId|SaltwaterType|SequenceType|MultiplesDivisorsType|SpeedType|WorkNewtonType|ProfitLossType|JudgementReasoningType; difficulty:'basic'; learningStage:LearningStage; question:string; formula:string; choices:string[]; answerIndex:number; shortRule:string; triggerWords:string[]; steps:string[]; explanation:string; deepExplanation:string; mistakeReason:string; diagramType:'sum-product-pair'|'square-tiles'|'difference-squares'|'saltwater-alligation'|'sequence-pattern'|'multiples-divisors'|'speed'|'work-newton'|'profit-loss'|'judgement-reasoning'; diagramData:Record<string,string>; tags:string[]; saltwater?:SaltwaterData; sequence?:SequenceData; multiplesDivisors?:MultiplesDivisorsData; speed?:SpeedData; workNewton?:WorkNewtonData; profitLoss?:ProfitLossData; judgementReasoning?:JudgementReasoningData };
