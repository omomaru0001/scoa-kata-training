...
const questions = [...(questionsJson as Question[]), ...saltwaterQuestions];
...
function App() {
  ...
  const [activeCategory, setActiveCategory] = useState<'factorization' | 'saltwater-alligation'>('factorization');
  ...
  const chooseCategory = (category: 'factorization' | 'saltwater-alligation') => { setActiveCategory(category); setIndex(0); setSelected(null); go('modes'); };
  ...
  if (screen === 'modes') return <main className="app"><ScreenHeader title="学習モードを選ぶ" onBack={() => goBack('categories')} /><p className="page-lead">{activeCategory === 'saltwater-alligation' ? '低い・目標・高いの順に置くところから始めます。' : 'まずは「まず覚える」から始めるのがおすすめです。'}</p><div className="mode-list">{modeItems.map(([icon, name, description], itemIndex) => <button key={name} className="selection-card mode-card" onClick={itemIndex === 5 ? () => go('weak') : begin}><span className="card-icon">{icon}</span><span className="card-copy"><b>{name}</b><small>{description}</small></span><span className="card-arrow">›</span></button>)}</div>{bottomNav}</main>;
